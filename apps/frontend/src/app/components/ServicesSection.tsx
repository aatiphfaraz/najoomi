...
export default function ServicesSection() {
  ...
  const [chunkSize, setChunkSize] = React.useState(3); // Always start with 3 to match SSR
  React.useEffect(() => {
  ...
  return (
    <section className="w-full bg-gradient-to-br from-white to-[#F1E1C6] py-10 md:py-20 px-2 md:px-4 flex flex-col items-center relative overflow-x-clip">
      ...
      <h2 className="text-2xl md:text-4xl font-bold text-[#15577a] mb-1 md:mb-2 z-10">
        Our Services
      </h2>
      <p className="mb-4 md:mb-10 text-gray-700 text-center max-w-xl md:max-w-2xl text-sm md:text-base z-10">
        Explore our spiritual services, asli Islamic tareeqe se, guidance,
        healing aur clarity paayein
      </p>
      ...
      <div className="block md:hidden w-full max-w-xl z-10 relative">
        ...
        >
          {serviceChunks.map((chunk, idx) => (
            <div
              className="keen-slider__slide px-0.5"
              key={idx}
              aria-label={`Service group ${idx + 1}`}
            >
        ...
        <div className="flex md:hidden justify-center gap-2 mt-4 z-10">
          {serviceChunks.map((_, idx) => (
            <button
              ...
              onClick={() =>
                sliderInstanceRef && sliderInstanceRef.moveToIdx(idx)
              }
              className={
                `w-3 h-3 rounded-full border border-[#b6894a] transition-all duration-200 ` +
                (currentSlide === idx
                  ? "bg-[#b6894a] shadow-[0_0_6px_#eab30855] scale-110"
                  : "bg-[#fef9c3] opacity-60")
              }
              style={{ outline: "none" }}
            >
              ...
              {currentSlide === idx && (
                ...
                >
                  <polygon
                    points="6,1 7,5 11,5 8,7 9,11 6,9 3,11 4,7 1,5 5,5"
                    fill="#eab308"
                    opacity="0.16"
                  />
                </svg>
      ...
      <div className="flex justify-center mt-6 md:mt-10 z-10">
        <Link
          href="/services"
          className="text-[#15577a] text-base md:text-lg font-semibold underline underline-offset-2 md:underline-offset-4 decoration-[#b6894a] hover:decoration-2 hover:text-[#b6894a] transition-all duration-150"
        >
          View All Services
        </Link>
      </div>
